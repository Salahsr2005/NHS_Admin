...
export function getRandomAvatar(gender: string | null, id: string): string {
  ...
  const { data } = supabase.storage.from("avatars").getPublicUrl(`${folder}/${avatarNum}.svg`)
      ...
      <CardContent className="p-6">
        ...
        <div className="flex flex-col items-center text-center mb-4">
          ...
          <h3 className="font-semibold text-foreground text-lg leading-tight mb-1">
            {getPersonDisplayName(applicant)}
          </h3>

          {applicant.age && <span className="text-xs text-muted-foreground">{applicant.age} years old</span>}
        </div>
        ...
        {skills.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            {skills.slice(0, 3).map((skill) => (
              <Badge key={skill} variant="outline" className="text-xs bg-secondary/50 border-border/50">
                {skill}
              </Badge>
            ))}
            {skills.length > 3 && (
